...
def init_model(model, args):
    input_images = tf.placeholder(dtype='float32', shape=(None, args.crop_size, args.crop_size, 8))
    ...
    for epoch in range(args.num_epochs):
        ...
        for i in range(num_batches):
            ...
            if iterations % args.print_every == 0:
                print(('{}: train f1_acc = {:.4f}, test f1_acc = {:.4f}, '
...
def main():
    ...
    train_generator = MULSpacenetDataset(
        ...
        seed=777,
        image_name_template='PS-MS/SN3_roads_train_AOI_2_Vegas_PS-MS_{id}.tif',
    ...
    # train_x in shape (batch_size, width, height, channels) = (train_batch_size, crop_size, crop_size, 8)
    train_x, train_y = next(train_generator)
    print(np.min(train_x[0]), np.max(train_x[0]), np.mean(train_x[0]))
    print(np.min(train_y[0]), np.max(train_y[0]), np.mean(train_y[0]))
